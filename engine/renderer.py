...
def render_viewport(
    ...
    log.debug(
        ...
        tile_arrays_len=(
            len(viewport.tile_arrays) if viewport.tile_arrays is not None else "None"
        ),
        fg_colors_shape=(
            viewport.tile_fg_colors.shape
            if viewport.tile_fg_colors is not None
            else "None"
        ),
        bg_colors_shape=(
            viewport.tile_bg_colors.shape
            if viewport.tile_bg_colors is not None
            else "None"
        ),
        indices_shape=(
            viewport.tile_indices_render.shape
            if viewport.tile_indices_render is not None
            else "None"
        ),
        coord_keys=(
            list(viewport.coord_arrays.keys())
    ...
    player_height = (
        0  # Default to 0 if player pos is somehow out of bounds for height map
    )
    try:
        if gm.in_bounds(player_x, player_y):
            ...
            log.error(
                "Player position out of map bounds during height lookup",
                player_pos=player_pos,
                map_shape=(gm.height, gm.width),
            )
            player_height = 0
    except IndexError:
        # Should not happen if in_bounds check passed, but defensive
        log.error(
            "IndexError getting player height in renderer",
            player_pos=player_pos,
            map_shape=(gm.height, gm.width),
        )
        player_height = 0
        ...
        log.error(
            "Unexpected error getting player height in renderer",
            error=str(e),
            exc_info=True,
            player_pos=player_pos,
        )
    ...
    try:
        ...
        if vp_h <= 0 or vp_w <= 0:
            log.warning(
                "Calculated actual viewport dimensions are zero or negative after slicing.",
                vp_h=vp_h,
                vp_w=vp_w,
            )
            ...
            return Image.new(
                "RGBA",
                (
                    max(1, viewport_width * max(1, tile_w)),
                    max(1, viewport_height * max(1, tile_h)),
                ),
                (0, 0, 0, 0),
            )
    except Exception as e:
        log.error(
            "Error during prepare_base_layers call or unpacking results",
            error=str(e),
            exc_info=True,
        )
    ...
    if output_pixel_h <= 0 or output_pixel_w <= 0:
        log.warning(
            "Calculated output pixel size is zero or negative",
            w=output_pixel_w,
            h=output_pixel_h,
        )
        # Return a minimal blank image
    ...
    try:
        ...
        # Set alpha channel to fully opaque for background
        output_image_array[:, :, 3] = 255
    except (KeyError, IndexError, ValueError) as e:
        log.error(
            "Error preparing output buffer with background (coordinate mapping)",
            error=str(e),
            exc_info=True,
            final_bg_shape=final_bg.shape if final_bg is not None else "None",
            output_shape=output_image_array.shape,
            coord_y_shape=coord_arrays.get("tile_coord_y", np.array([])).shape,
            coord_x_shape=coord_arrays.get("tile_coord_x", np.array([])).shape,
            vp_dims=(vp_h, vp_w),
        )
        ...
        log.error(
            "Unexpected error during background fill", error=str(e), exc_info=True
        )
        return Image.new(
            "RGBA", (max(1, output_pixel_w), max(1, output_pixel_h)), (100, 0, 100, 255)
        )
    ...
    try:
        ...
        if not tile_arrays:
            ...
            log.debug(
                "render_map_tiles skipped: drawn_mask is all False (no tiles to draw)."
            )
            ...
            render_map_tiles(
                ...
                drawn_mask,
                final_fg,
                final_bg,  # final_bg passed for potential fallback
                tile_arrays,
                vp_h,
                vp_w,
                tile_w,
            ...
            log.warning(
                "render_map_tiles skipped: Numba not available or tile_arrays is not NumbaDict."
            )
        ...
        log.error(
            "Error during render_map_tiles Numba call", error=str(e), exc_info=True
        )
        ...
        try:
            ...
            if items_in_vp_df.height > 0:
                ...
                if np.any(valid_indices_mask):
                    ...
                    valid_item_rel_y_np = item_coords_y_vp[valid_indices_mask].astype(
                        np.intp
                    )  # Use .astype(np.intp) for indexing
                    valid_item_rel_x_np = item_coords_x_vp[valid_indices_mask].astype(
                        np.intp
                    )  # Use .astype(np.intp) for indexing
                    ...
                    if map_visible_vp.shape == (vp_h, vp_w):
                        ...
                        visible_item_mask[valid_indices_mask] = map_visible_vp[
                            valid_item_rel_y_np, valid_item_rel_x_np
                        ]
                    else:
                        log.warning(
                            "map_visible_vp shape mismatch, cannot accurately check item visibility.",
                            map_vis_shape=map_visible_vp.shape,
                            expected=(vp_h, vp_w),
                        )
                        # Fallback: Assume visible if coordinates are valid for the viewport data
                        # Less accurate, but prevents skipping all items
                        visible_item_mask[valid_indices_mask] = True
            ...
            log.error(
                "Error querying/processing ground items for rendering",
                error=str(e),
                exc_info=True,
            )
    ...
    if item_xs is not None and item_xs.size > 0:
        try:
            ...
            log.error(
                "Error during render_ground_items Numba call",
                error=str(e),
                exc_info=True,
            )
        ...
        try:
            ...
            if active_entities_df.height > 0:
                entities_in_vp_bounds = active_entities_df.filter(
                    ...
                    &
                    # Use vp_y_min/max from item query for consistency
                    (pl.col("y") >= vp_y_min)
                    & (pl.col("y") < vp_y_max)
                    &
                    # Only include entities with a valid glyph
                    (pl.col("glyph") > 0)
                )
            ...
            log.error(
                "Error querying/processing entities for rendering",
                error=str(e),
                exc_info=True,
            )
    ...
    if entity_xs is not None and entity_xs.size > 0:
        try:
            ...
            log.error(
                "Error during render_entities Numba call", error=str(e), exc_info=True
            )
    ...
    try:
        ...
        if output_pixel_h > 0 and output_pixel_w > 0:
            ...
            log.warning(
                "Output pixel dimensions were non-positive after processing.",
                w=output_pixel_w,
                h=output_pixel_h,
            )
            # Return a minimal blank image
        ...
        log.error(
            "Error converting final array to PIL Image", error=str(e), exc_info=True
        )
