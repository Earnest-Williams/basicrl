...
def render_viewport(
    ...
    try:
        if gm.in_bounds(player_x, player_y):
            ...
            log.error("Player position out of map bounds during height lookup",
                      player_pos=player_pos, map_shape=(gm.height, gm.width))
            player_height = 0
    except IndexError:
        # Should not happen if in_bounds check passed, but defensive
        log.error("IndexError getting player height in renderer",
                  player_pos=player_pos, map_shape=(gm.height, gm.width))
        player_height = 0
    except Exception as e:
        # Catch any other unexpected errors during height lookup
        log.error("Unexpected error getting player height in renderer",
                  error=str(e), exc_info=True, player_pos=player_pos)
        player_height = 0
    ...
    try:
        ...
        if vp_h <= 0 or vp_w <= 0:
            log.warning(
                "Calculated actual viewport dimensions are zero or negative after slicing.", vp_h=vp_h, vp_w=vp_w)
        ...
        log.error("Error during prepare_base_layers call or unpacking results", error=str(
            e), exc_info=True)
    ...
    if output_pixel_h <= 0 or output_pixel_w <= 0:
        log.warning("Calculated output pixel size is zero or negative",
                    w=output_pixel_w, h=output_pixel_h)
        # Return a minimal blank image
        return Image.new("RGBA", (1, 1), (0, 0, 0, 0))
    output_image_array = np.zeros(
        (output_pixel_h, output_pixel_w, 4), dtype=np.uint8)
    ...
    try:
        ...
        if tile_coord_y.shape != (output_pixel_h, output_pixel_w) or tile_coord_x.shape != (output_pixel_h, output_pixel_w):
            raise ValueError(f"Coordinate array shape mismatch. Expected:{
        ...
        if final_bg.shape != (vp_h, vp_w, 3):
            raise ValueError(f"Background color shape mismatch. Expected:{
        ...
        log.error("Error preparing output buffer with background (coordinate mapping)", error=str(e), exc_info=True,
                  ...
                  coord_y_shape=coord_arrays.get("tile_coord_y", np.array(
                      [])).shape, coord_x_shape=coord_arrays.get("tile_coord_x", np.array([])).shape,
                  vp_dims=(vp_h, vp_w))
        ...
        log.error("Unexpected error during background fill",
    ...
    try:
        ...
        if not tile_arrays:
            log.warning(
                "render_map_tiles skipped: tile_arrays dictionary is empty.")
        ...
        log.error("Error during render_map_tiles Numba call",
                  error=str(e), exc_info=True)
    ...
    if np.any(map_visible_vp):
        try:
            ...
            if items_in_vp_df.height > 0:
                ...
                valid_indices_mask = (item_coords_x_vp >= 0) & (item_coords_x_vp < vp_w) & (
                    item_coords_y_vp >= 0) & (item_coords_y_vp < vp_h)
                ...
                if np.any(valid_indices_mask):
                    ...
                    valid_item_rel_y_np = item_coords_y_vp[valid_indices_mask].astype(
                        np.intp)  # Use .astype(np.intp) for indexing
                    ...
                    if map_visible_vp.shape == (vp_h, vp_w):
                        ...
                        visible_item_mask[valid_indices_mask] = map_visible_vp[valid_item_rel_y_np,
                                                                               valid_item_rel_x_np]
                    else:
                        log.warning("map_visible_vp shape mismatch, cannot accurately check item visibility.",
                                    map_vis_shape=map_visible_vp.shape, expected=(vp_h, vp_w))
                ...
                if visible_items_df.height > 0:
                    ...
                    items_to_render_list = visible_items_df.select(
                        ["x", "y", "glyph", "color_fg_r",
                            "color_fg_g", "color_fg_b", "item_id"]
                    ).to_dicts()

        except Exception as e:
            # Catch any errors during Polars query or data processing for items
            log.error("Error querying/processing ground items for rendering",
                      error=str(e), exc_info=True)
    ...
    if items_to_render_list:
        try:
            ...
            log.error("Error during render_ground_items Numba call",
                      error=str(e), exc_info=True)
            ...
            if active_entities_df.height > 0:
                ...
                if entities_in_vp_bounds.height > 0:
                    ...
                    entities_to_render_list = entities_in_vp_bounds.select(
                        ["entity_id", "x", "y", "glyph",
                            "color_fg_r", "color_fg_g", "color_fg_b"]
                    ).to_dicts()

        except Exception as e:
            # Catch any errors during Polars query or data processing for entities
            log.error("Error querying/processing entities for rendering",
                      error=str(e), exc_info=True)
    ...
    if entities_to_render_list:
        try:
            ...
            log.error("Error during render_entities Numba call",
                      error=str(e), exc_info=True)
    ...
    try:
        ...
        log.error("Error converting final array to PIL Image",
                  error=str(e), exc_info=True)
