...
def test_combat_error_propagates(monkeypatch):
    ...
    er.create_entity(x=6, y=5, glyph=ord(
        'g'), color_fg=(255, 0, 0), name='Goblin')
...
def test_fall_damage_message_visibility():
    ...
    entity_id = er.create_entity(x=x, y=start_y, glyph=ord(
        'g'), color_fg=(255, 0, 0), name='Goblin', hp=10, max_hp=10)
    ...
    action_handler._handle_fall(
        entity_id, (x, start_y, 10), (x, start_y + 1, 7), gs, max_step=1)
    assert not any('falling damage' in msg[0].lower()
                   for msg in gs.message_log)
...
def test_fall_death_cleanup():
    ...
    _setup_fall_path(gs.game_map, x, start_y)
    entity_id = er.create_entity(x=x, y=start_y, glyph=ord(
        'g'), color_fg=(255, 0, 0), name='Goblin', hp=5, max_hp=5)
    ir.create_item('test_item', location='inventory',
