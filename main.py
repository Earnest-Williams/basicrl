...
def print_map_section(game_map: GameMap, center_x: int, center_y: int, radius: int = 5):
    ...
    if not isinstance(game_map, GameMap):
        log.warning("Cannot print map section: Invalid GameMap object.")
        return
    y_min = max(0, center_y - radius)
    y_max = min(game_map.height, center_y + radius + 1)
    x_min = max(0, center_x - radius)
    x_max = min(game_map.width, center_x + radius + 1)
    ...
    for y in range(y_min, y_max):
        ...
        for x in range(x_min, x_max):
            char = "?"
            tile_id = game_map.tiles[y, x]
            if tile_id == TILE_ID_FLOOR:
                char = "."
            elif tile_id == TILE_ID_WALL:
                char = "#"
            else:
                char = str(tile_id)
