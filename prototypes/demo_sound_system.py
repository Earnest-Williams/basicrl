...
def main():
    ...
    result = play_sound("player_damaged", {"target": "player", "damage_type": "physical"})
    print(f"      Result: {'✅ Played' if result else '❌ Failed/Disabled'}")
    ...
    handle_event("deal_damage", {"target": "player", "damage_type": "fire", "amount": 5})
    ...
    print(f"      Original volumes - Master: {original_master}, SFX: {original_sfx}, Music: {original_music}")
    
    sound_manager.set_master_volume(0.5)
    sound_manager.set_sfx_volume(0.3)
    sound_manager.set_music_volume(0.8)
    
    print(f"      New volumes - Master: {sound_manager.master_volume}, SFX: {sound_manager.sfx_volume}, Music: {sound_manager.music_volume}")
    ...
    for name in sorted(sound_manager.background_music.keys()):
        ...
        print(f"      • {name} (priority: {priority}, conditions: {', '.join(conditions)})")
